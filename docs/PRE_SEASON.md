...
| 부수 처리 | 선택 즉시 `StffHireService.onBrdcstSelected()` 호출 → 감독 6명·코치 10명 후보 생성 (`STFF_CAND`), `GAME_CFG(STFF_HIRED=0)` 리셋 |
| 감독·코치 선임 버튼 | 방송국 선택 완료 & `STFF_HIRED=0` 조건 시 SeasonPage 상단에 "감독·코치 선임하기" 버튼 표시 |
...
| 항목 | 내용 |
|------|------|
| 트리거 | 방송국 선택 완료 후 SeasonPage 상단 "감독·코치 선임하기" 버튼 표시 (`STFF_HIRED=0` 상태) |
| UI | 상단 버튼 또는 뉴스 STFF 이벤트 하단 버튼 클릭 → `StaffHireModal` |
